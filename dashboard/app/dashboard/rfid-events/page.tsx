...
export default function RfidEventsPage() {
  ...
  return <section><h2>Live RFID Events</h2><p>Automatically refreshed every 5 seconds.</p>{error && <p role="alert">{error}</p>}<table><thead><tr><th>Time</th><th>Node</th><th>Checkpoint</th><th>Employee</th><th>UID</th><th>Status</th><th>LoRa</th></tr></thead><tbody>{events.map(event => <tr key={event.id}><td>{new Date(event.timestamp).toLocaleString()}</td><td>{event.node_id}</td><td>{event.checkpoint_id ?? "—"}</td><td>{event.employee_id || "Unknown"}</td><td>{event.uid}</td><td style={event.status === "INVALID" ? { color: "#b91c1c", fontWeight: 600 } : undefined}>{event.status}{event.reason && <div style={{ fontSize: "0.8em", fontWeight: 400 }}>{event.reason}</div>}</td><td>{event.rssi ?? "—"} dBm / {event.snr ?? "—"} dB</td></tr>)}</tbody></table></section>;
}
