...
bool Inmp441Capture::begin(int bclkPin, int wsPin, int dataPin, int sampleRate, int shift, SampleRing* ring) {
    ...
    if (i2s_driver_install(I2S_NUM_0, &cfg, 0, nullptr) != ESP_OK) {
        Serial.println("[ACOUSTIC] i2s_driver_install failed");
        return false;
    }
    ...
    if (i2s_set_pin(I2S_NUM_0, &pins) != ESP_OK) {
        Serial.println("[ACOUSTIC] i2s_set_pin failed");
        return false;
    }
...
int Inmp441Capture::pump(uint32_t timeoutMs) {
    if (!started_ || ring_ == nullptr) return -1;
    int32_t raw[128];
    int16_t pcm[128];
    size_t bytes = 0;
    if (i2s_read(I2S_NUM_0, raw, sizeof(raw), &bytes, pdMS_TO_TICKS(timeoutMs)) != ESP_OK) return -1;
